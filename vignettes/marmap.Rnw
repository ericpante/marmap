...
Alternatively, it is possible to import two compatible \verb;bathy; objects (for instance from GEBCO), one for the eastern part and one for the western part of the area of interest. The function \verb;collate.bathy; takes care of the stitching process: relabelling longitudes in the 0-360 degrees range, removing duplicated data (is.e. the data for longitude 180 is often present once in each individual dataset and needs to be removed once), etc. Providing that we downloaded two files east.nc and west.nc from the GEBCO website, creating a proper bathy object for the antimeridian region is simple:
...
\verb"getNOAA.bathy()" is the easiest way to load data into R, but it depends on the NOAA download protocol, and one must have an internet connection (see above). Setting the \verb;keep; argument to \verb;TRUE; will save on disk the data downloaded from the NOAA servers. This allows the users to run scripts without having to download the same data again and again, and without having to modify the script since \verb;getNOAA.bathy; scans the current working directory for bathy objects already available before fetching data on NOAA servers. \verb"read.bathy()" allows import of data into R, and this data can be located on a drive ; an internet connection is therefore not mandatory. This is a good way to import data that have been saved locally on your drive, and may be faster than re-downloading data from the NOAA server at the beginning of each R session. If the user is building maps routinely, we propose two functions to create a local database that can be accessed from within R. These functions are \verb"setSQL()" and \verb"subsetSQL()". \\
