...
if(is.null(coord)){
	...
	mm.pointLabels = function (x, y = NULL, labels = seq(along = x), cex = 1, method = c("SANN",
	    "GA"), allowSmallOverlap = FALSE, trace = FALSE, doPlot = TRUE, ...)
	{
	...
	#########################################################################################
		...
		mm.pointLabels(x,y, cex=pie.radius*pie.space) -> xy
	   	...
	   	if(link==TRUE){
	   		segments(x,y, xy$x, xy$y, lwd=seg.lwd,col=seg.col, lty=seg.lty)
	    	}


		for(i in 1:length(xy$x)){
	        	plotrix::floating.pie(xy$x[i], xy$y[i], x=as.numeric(pie.slices[i,][pie.slices[i,]!=0]),
	        		col=as.vector(pie.colors[i,][pie.colors[i,]!=0]), radius=pie.radius, shadow=F)
	   	}
   	} # end of if statement
