getBSHC.bathy <- function (lon1, lon2, lat1, lat2, keep = FALSE) 
{
	...
	if(any(c(x1,x2) < 33) | any(c(x1,x2) > 39)) stop("At least one of your query longitudes is outside the BSHC range (33-39)")
	if(any(c(y1,y2) < 49) | any(c(y1,y2) > 55)) stop("At least one of your query latitudes is outside the BSHC range (49-55) ")

    fetch <- function(x1, y1, x2, y2, res) {
        WEB.REQUEST <- paste("http://data.bshc.pro/ogc/bsbd-0.9.3?SERVICE=wcs&VERSION=1.0.0&REQUEST=GetCoverage&coverage=bsbd&CRS=EPSG:4326&bbox=", y1, ",", x1, ",", y2, ",", x2,"&width=", w, "&height=", h, "&format=XYZ", sep = "")
        
        cat(paste(WEB.REQUEST,"\n"))
